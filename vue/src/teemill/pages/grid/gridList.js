...
var GridList = function(items, options, element) {
  /**
...
GridList.prototype = {
  ...
  snapItemPositionToGrid: function(position, size) {
    const relativePosition = [
      position[0] - this.element.offsetLeft,
      position[1] - this.element.offsetTop,
    ];

    var col = Math.round(relativePosition[0] / 200),
        row = Math.round(relativePosition[1] / 200);
    ...
    if (this._options.direction === "horizontal") {
      ...
      row = Math.min(row, this._options.lanes - size[1]);
    } else {
      col = Math.min(col, this._options.lanes - size[0]);
